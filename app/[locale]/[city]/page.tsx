...
export default async function CityPage({ params }: { params: Promise<{ locale: string; city: string }> }) {
  ...
  const copy =
    locale === 'it'
      ? {
          weeklyClasses: 'Programmi attivi',
          ...
          teachersTitle: 'Le persone dietro le attivita.',
          teachersLead: 'Persone, non solo slot.',
          openTeachers: 'Apri elenco completo'
        }
      : {
          weeklyClasses: 'Active programs',
          ...
          teachers: 'Organizers',
          teachersTitle: 'People behind the activities.',
          ...
          <div className="panel">
            ...
            <div className="card-grid">
              {instructors.slice(0, 4).map((instructor) => (
                <ServerCardLink key={instructor.slug} href={`/${locale}/${citySlug}/organizers/${instructor.slug}`} className="collection-card">
