...
postgresLines.push(`INSERT INTO cities (slug, country_code, timezone, status, bounds, name, hero) VALUES (${sqlValue('palermo')}, ${sqlValue('IT')}, ${sqlValue('Europe/Rome')}, ${sqlValue('public')}, ${jsonSql([13.2805, 38.085, 13.405, 38.165])}, ${jsonSql(localized('Palermo', 'Palermo'))}, ${jsonSql(localized('The citywide family activity guide for Palermo.', 'La guida cittadina alle attivita per famiglie a Palermo.'))}) ON CONFLICT (slug) DO NOTHING;`);
postgresLines.push(`INSERT INTO cities (slug, country_code, timezone, status, bounds, name, hero) VALUES (${sqlValue('catania')}, ${sqlValue('IT')}, ${sqlValue('Europe/Rome')}, ${sqlValue('seed')}, ${jsonSql([15.02, 37.45, 15.18, 37.57])}, ${jsonSql(localized('Catania', 'Catania'))}, ${jsonSql(localized('Next city in the chefamo pipeline.', 'La prossima citta nella pipeline chefamo.'))}) ON CONFLICT (slug) DO NOTHING;`);
